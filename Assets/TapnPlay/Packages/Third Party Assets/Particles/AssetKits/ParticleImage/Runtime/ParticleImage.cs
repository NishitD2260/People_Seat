...
namespace AssetKits.ParticleImage
{
    ...
    public sealed class ParticleImage : MaskableGraphic
    {
        ...
        [SerializeField]
        private ParticleSystem.MinMaxGradient _startColor = new ParticleSystem.MinMaxGradient(
            Color.white
        );
        ...
        private ParticleSystem.MinMaxGradient _colorOverLifetime =
            new ParticleSystem.MinMaxGradient(new Gradient());
        ...
        [SerializeField]
        private ParticleSystem.MinMaxGradient _colorBySpeed = new ParticleSystem.MinMaxGradient(
            new Gradient()
        );
        ...
        [SerializeField]
        private SeparatedMinMaxCurve _sizeOverLifetime = new SeparatedMinMaxCurve(
            new AnimationCurve(new[] { new Keyframe(0f, 1f), new Keyframe(1f, 1f) })
        );
        ...
        [SerializeField]
        private SeparatedMinMaxCurve _sizeBySpeed = new SeparatedMinMaxCurve(
            new AnimationCurve(new[] { new Keyframe(0f, 1f), new Keyframe(1f, 1f) })
        );
        ...
        [SerializeField]
        private SeparatedMinMaxCurve _rotationBySpeed = new SeparatedMinMaxCurve(
            new AnimationCurve(new[] { new Keyframe(0f, 1f), new Keyframe(1f, 1f) })
        );
        ...
        [SerializeField]
        private ParticleSystem.MinMaxCurve _toTarget = new ParticleSystem.MinMaxCurve(
            1f,
            new AnimationCurve(new[] { new Keyframe(0f, 0f), new Keyframe(1f, 1f) })
        );
        ...
        [SerializeField]
        private SeparatedMinMaxCurve _velocityOverLifetime = new SeparatedMinMaxCurve(
            0f,
            true,
            false
        );
            ...
            get
            {
                if (_pool == null)
                {
                    _pool = new ParticlePool(
                        (int)(_rate + _rateOverLifetime + _rateOverDistance),
                        this
                    );
                }
        ...
        public NativeList<FixedList4096Bytes<TrailPointData>> TrailData
        {
            get
            {
                if (trailData.IsCreated == false && multithreadEnabled)
                    trailData = new NativeList<FixedList4096Bytes<TrailPointData>>(
        ...
        [SerializeField]
        private ParticleSystem.MinMaxCurve _trailWidth = new ParticleSystem.MinMaxCurve(
            1f,
            new AnimationCurve(new[] { new Keyframe(0f, 1f), new Keyframe(1f, 0f) })
        );
        ...
        [SerializeField]
        private ParticleSystem.MinMaxGradient _trailColorOverLifetime =
        ...
        [SerializeField]
        private ParticleSystem.MinMaxGradient _trailColorOverTrail =
            new ParticleSystem.MinMaxGradient(Color.white);
        ...
        [SerializeField]
        private bool _dieWithParticle = false;
        ...
        {
            get { return _multithreadModule.enabled && _multithreadEnabled; }
            set
            {
                if (isMain && children != null)
                {
        ...
        private void DoPlay()
        {
            ...
            Simulate(
                _timeScale == TimeScale.Normal ? Time.deltaTime : Time.unscaledDeltaTime,
                prewarm
            );
                ...
                if (multithreadEnabled)
                {
                    ...
                    for (int i = -w1; i < w1; i++)
                    {
                        for (int j = -h1; j < h1; j++)
                        {
                            ...
                            {
                                n = Unity.Mathematics.noise.snoise(
                                    new float3(p + new Vector3(noiseOffset.x, noiseOffset.y, 0)).xy
                                        * noiseFrequency
                                        / 100f
                                );
                            }
                            else
                            {
                                ...
                                n = Unity.Mathematics.noise.snoise(
                                    new float3(po + transform.localPosition).xy
                                        * noiseFrequency
                                        / 100f
                                );
                            }
                            ...
                            Debug.DrawLine(
                                RotatePointAroundCenter(
                                    (p + transform.localPosition) * canvasRect.localScale.x,
                                    canvasRect.eulerAngles
                                ) + canvasRect.position,
                                RotatePointAroundCenter(
                                    (
                                        p
                                ...
                                new Color(n, n.Remap(-1f, 1f, 1f, 0f), 0f)
                            );
                        }
                    }
                    return;
                ...
                for (int i = -w; i < w; i++)
                {
                    for (int j = -h; j < h; j++)
                    {
                        ...
                        if (space == Simulation.World)
                            no = _noise.GetNoise(
                                pos.x + transform.localPosition.x + noiseOffset.x,
                                pos.y + transform.localPosition.y + noiseOffset.y
                            );

                        Debug.DrawLine(
                            RotatePointAroundCenter(
                                (pos + transform.localPosition) * canvasRect.localScale.x,
                                canvasRect.eulerAngles
                            ) + canvasRect.position,
                            RotatePointAroundCenter(
                                (
        ...
        {
            sheetsArray = new NativeArray<SpriteSheet>(
                textureTile.x * textureTile.y,
                multithreadEnabled ? Allocator.TempJob : Allocator.Temp
            );
            ...
            if (textureSheetEnabled)
            {
                for (int i = textureTile.y - 1; i > -1; i--)
                {
                    ...
                    {
                        sheetsArray[(textureTile.y - 1 - i) * textureTile.x + j] = new SpriteSheet(
            ...
            else
            {
                if (sprite != null)
                {
                    ...
                    sheetsArray[0] = new SpriteSheet(
                        new Vector2(rect[2], rect[3]),
                        new Vector2(rect[0], rect[1])
                    );
                }
                else
        ...
        private void Simulate(float deltaTime, bool prewarming = false)
        {
            ...
            if (isEmitting && !canvasRenderer.cull)
            {
                ...
                if (_bursts != null)
                {
                    ...
                    {
                        if (
                            _burstTimer >= _bursts[i].time + _startDelay && _bursts[i].used == false
                        )
                        {
            ...
            if (isPlaying)
            {
                ...
                if (_emitterConstraintTransform && _emitterConstraintEnabled.enabled)
                {
                    ...
                    {
                        Vector3 canPos;
                        Vector3 viewportPos = mainCamera.WorldToViewportPoint(
                            _emitterConstraintTransform.position
                        );
                ...
                if (multithreadEnabled)
                {
                    ...
                    _meshData.SetVertexBufferParams(
                        JobParticles.Length * 4,
                                ...
                                targetPos = new Vector3(
                                    (
                                        (
                                            ...
                                            - canvasRect.InverseTransformPoint(transform.position).x
                                            + canvasRect.localPosition.x
                                    ...
                                    (
                                        (
                                            ...
                                            - canvasRect.InverseTransformPoint(transform.position).y
                                            + canvasRect.localPosition.y
                    ...
                    if (space == Simulation.World)
                    {
                        worldSpaceCounterPos = transform.InverseTransformPoint(
                            _lastTransformPosition
                        );

                        _transformDeltaRotation =
                            Quaternion.Inverse(transform.rotation).eulerAngles
                            - Quaternion.Inverse(_lastTransformRotation).eulerAngles;
                    ...
                    _particleJob = new ParticleJob()
                    {
                        ...
                        sizeOverLifetime = JobUtils.ConvertSeparatedMinMaxCurveJobData(
                            sizeOverLifetime
                        ),
                        ...
                        rotationOverLifetime = JobUtils.ConvertSeparatedMinMaxCurveJobData(
                            rotationOverLifetime
                        ),
                        rotationBySpeed = JobUtils.ConvertSeparatedMinMaxCurveJobData(
                            rotationBySpeed
                        ),
                        ...
                        velocityOverLifetime = JobUtils.ConvertSeparatedMinMaxCurveJobData(
                        ...
                        textureSheetFrameOverTime = JobUtils.ConvertMinMaxCurveToJobs(
                            textureSheetFrameOverTime
                        ),
                        textureSheetStartFrame = JobUtils.ConvertMinMaxCurveToJobs(
                    ...
                    if (trailsEnabled && _particleTrailRenderer)
                    {
                        ...
                        _trailMeshData.SetVertexBufferParams(
                            totalTrailPoints,
                            new VertexAttributeDescriptor(VertexAttribute.Position),
                            new VertexAttributeDescriptor(
                                VertexAttribute.Color,
                        ...
                        _trailJob = new TrailJob()
                        {
                            ...
                            trailColorOverLifetime = JobUtils.ConvertMinMaxColorJobData(
                                _trailColorOverLifetime
                            ),
                            trailColorOverTrail = JobUtils.ConvertMinMaxColorJobData(
                                _trailColorOverTrail
                            )
                        };

                        _trailJobHandle = _trailJob.Schedule(
                            JobParticles.Length,
                            16,
                            _particleJobHandle
                        );
                    ...
                    if (trailsEnabled && _particleTrailRenderer)
                        _particleTrailRenderer.SetMeshData(
                            _trailMeshDataArray,
                            _trailMeshData,
                            totalIndices
                        );
                    ...
                    if (JobParticles.Length > 0)
                    {
                        ref readonly ParticleData lastParticle = ref JobParticles.ElementAt(
                            JobParticles.Length - 1
                        );
                        while (
                            lastParticle.time >= lastParticle.lifeTime
                            && (
                                !trailsEnabled
                                || (
                                    TrailData.ElementAt(JobParticles.Length - 1).Length <= 1
                                    || _dieWithParticle
                                )
                            )
                        )
                ...
                for (int i = particles.Count - 1; i >= 0; i--)
                {
                    ...
                    uvs[vert + 1] = new Vector2(
                        sheetsArray[particles[i].GetSheetId].pos.x,
                        sheetsArray[particles[i].GetSheetId].size.y
                    );
                    uvs[vert + 2] = sheetsArray[particles[i].GetSheetId].pos;
                    uvs[vert + 3] = new Vector2(
                        sheetsArray[particles[i].GetSheetId].size.x,
                        sheetsArray[particles[i].GetSheetId].pos.y
                    );
                ...
                {
                    if (
                        _particles[i].TimeSinceBorn > _particles[i].Lifetime
                        && (_particles[i].trailPoints.Count <= 1 || _dieWithParticle)
                    )
                    {
                        OnAnyParticleFinished();
        ...
        private ParticleData GenerateJobParticle(int order, int source, Burst burst)
        {
            ...
            switch (_shape)
            {
                ...
                case EmitterShape.Circle:
                    ...
                    if (_emitOnSurface)
                    {
                        ...
                        {
                            startPosition =
                                RotateOnAngleFloat2(new float2(0, Random.Range(0f, 1f)), angle)
                                * _radius;
                        }
                    }
                    else
                    {
                        if (_spread == SpreadType.Random)
                        {
                            ...
                            startPosition = math.lerp(
                                r * _radius,
                                r * (_radius - _emitterThickness),
                                Random.value
                            );
                        }
                        else
                        {
                            startPosition =
                                RotateOnAngleFloat2(new float2(0, 1f), angle)
                ...
                case EmitterShape.Rectangle:
                    if (_emitOnSurface)
                    {
                        ...
                        {
                            startPosition = math.lerp(
                                GetPointOnRectFloat2(math.radians(angle), _width, _height),
                                new float2(1f),
                                Random.value
                            );
                        }
                        else
                        {
                            startPosition = new float2(
                                Random.Range(-_width / 2, _width / 2),
                    ...
                    else
                    {
                        ...
                        startPosition = math.lerp(
                            GetPointOnRectFloat2(math.radians(a), _width, _height),
                            GetPointOnRectFloat2(
                                math.radians(a),
                                _width - _emitterThickness,
                                _height - _emitterThickness
                            ),
                            Random.value
                        );
                    }
                ...
                case EmitterShape.Line:
                    ...
                    {
                        startPosition = new float2(
                            Mathf.Repeat(angle, 361).Remap(0, 360, -_length / 2, _length / 2),
                            0
                        );
                    }
                    else
            ...
            switch (_shape)
            {
                case EmitterShape.Point:
                    ...
                    {
                        startVelocity =
                            math.normalize(new float2(rp.x, rp.y))
                            * _startSpeed.Evaluate(Random.value, Random.value);
                    }
                    break;
                case EmitterShape.Circle:
                    startVelocity =
                        math.normalize(startPosition)
                        * _startSpeed.Evaluate(Random.value, Random.value);
                    break;
                case EmitterShape.Rectangle:
                    startVelocity =
                ...
                case EmitterShape.Line:
                    startVelocity =
                        (
                            space == Simulation.World
                                ? new float2(transform.up.x, transform.up.y)
                                : new float2(Vector3.up.x, Vector3.up.y)
                        ) * _startSpeed.Evaluate(Random.value, Random.value);
                    break;
                case EmitterShape.Directional:
                    float a = 0;
                    if (space == Simulation.World)
                    {
                        ...
                        {
                            a =
                                Mathf.Repeat(angle, 361).Remap(0, 360, -_angle / 2, _angle / 2)
                    ...
                    startVelocity =
                        RotateOnAngleFloat2(a) * _startSpeed.Evaluate(Random.value, Random.value);
                    break;
            }
            ...
            if (trailsEnabled)
            {
                if (TrailData.Length == 0)
                {
                    ...
                    initialTrailPoint.Add(
                        new TrailPointData() { position = (half2)startPosition, time = (half)0, }
                    );
            ...
            particle.frameId = (int)
                textureSheetStartFrame.Evaluate(
                    _time.Remap(0f, particle.lifeTime, 0f, 1f),
                    Random.value
                );
        ...
        private Particle GenerateParticle(int order, int source, Burst burst, float startTime)
        {
            ...
            switch (_shape)
            {
                ...
                case EmitterShape.Circle:
                    if (_emitOnSurface)
                    {
                        ...
                        {
                            startPosition =
                                RotateOnAngle(new Vector3(0, Random.Range(0f, 1f), 0), angle)
                                * _radius;
                        }
                    }
                    else
                    {
                        ...
                        {
                            startPosition =
                                RotateOnAngle(new Vector3(0, 1f, 0), angle)
                                * (UnityEngine.Random.Range(_radius, _radius - _emitterThickness));
                        }
                    }
                    break;
                case EmitterShape.Rectangle:
                    if (_emitOnSurface)
                    {
                        if (_spread == SpreadType.Uniform)
                        {
                            startPosition = Vector2.Lerp(
                                GetPointOnRect(angle * Mathf.Deg2Rad, _width, _height),
                                Vector2.one,
                                Random.value
                            );
                        }
                    ...
                    else
                    {
                        ...
                        startPosition = Vector2.Lerp(
                            GetPointOnRect(a * Mathf.Deg2Rad, _width, _height),
                            GetPointOnRect(
                                a * Mathf.Deg2Rad,
                                _width - _emitterThickness,
                                _height - _emitterThickness
            ...
            switch (_shape)
            {
                ...
                case EmitterShape.Circle:
                    startVelocity =
                        startPosition.normalized * _startSpeed.Evaluate(Random.value, Random.value);
                    break;
                case EmitterShape.Rectangle:
                    startVelocity =
                        startPosition.normalized * _startSpeed.Evaluate(Random.value, Random.value);
                ...
                case EmitterShape.Line:
                    startVelocity =
                        (space == Simulation.World ? transform.up : Vector3.up)
                        * _startSpeed.Evaluate(Random.value, Random.value);
                    break;
                case EmitterShape.Directional:
                    float a = 0;
                    ...
                    startVelocity =
                        RotateOnAngle(a) * _startSpeed.Evaluate(Random.value, Random.value);
                    break;
            }
    ...
    public struct SeparatedMinMaxCurve
    {
        ...
        public SeparatedMinMaxCurve(
            AnimationCurve startValue,
        ...
        public Vector3 Evaluate(float time, float lerp)
        {
            ...
            {
                return new Vector3(
                    xCurve.Evaluate(time, lerp),
                    yCurve.Evaluate(time, lerp),
                    zCurve.Evaluate(time, lerp)
                );
            }

            return new Vector3(
                mainCurve.Evaluate(time, lerp),
                mainCurve.Evaluate(time, lerp),
                mainCurve.Evaluate(time, lerp)
            );
        }
    ...
    public static class Extensions
    {
        public static float Remap(this float value, float from1, float to1, float from2, float to2)
        {
            ...
            if (
                math.abs(to1 - from1)
                < math.max(0.000001f * math.max(math.abs(from1), math.abs(to1)), math.EPSILON * 8)
            )
            {
                return to1;
            }
